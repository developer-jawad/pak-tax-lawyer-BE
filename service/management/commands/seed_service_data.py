...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created ServiceSection: {service_section.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"ServiceSection already exists: {service_section.title}"
                )
            )
        ...
        hero, created = ServiceHero.objects.get_or_create(
            ...
            defaults={"subtitle": hero_data.get("subtitle")},
        )
        if created:
        ...
        for stat_data in data.get("statistics", []):
            statistic = ServiceStatistic.objects.create(
                number=stat_data.get("number"), label=stat_data.get("label")
            )
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created ServiceStatistic: {statistic.number} - {statistic.label}"
                )
            )
        ...
        for benefit_data in data.get("benefits", []):
            benefit = ServiceBenefit.objects.create(
                icon=benefit_data.get("icon"), text=benefit_data.get("text")
            )
            self.stdout.write(
                self.style.SUCCESS(f"Created ServiceBenefit: {benefit.text}")
            )
