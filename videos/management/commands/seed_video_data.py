...
from videos.models import Video, VideoCTA, VideoHero, VideoSection, VideoStatistic
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created VideoHero: {video_hero.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"VideoHero already exists: {video_hero.title}")
            )
        ...
        for stat_data in hero_statistics_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created VideoStatistic: {statistic.number} - {statistic.label}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"VideoStatistic already exists: {statistic.number} - {statistic.label}"
                    )
                )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created VideoSection: {video_section.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"VideoSection already exists: {video_section.title}"
                )
            )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created VideoCTA: {video_cta.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"VideoCTA already exists: {video_cta.title}")
            )
