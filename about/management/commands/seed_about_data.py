...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created AboutHero: {about_hero.title}'))
        else:
            self.stdout.write(self.style.WARNING(f'AboutHero already exists: {about_hero.title}'))
        ...
        for stat_data in hero_statistics_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created AboutStatistic: {statistic.number} - {statistic.label}'))
            else:
                self.stdout.write(self.style.WARNING(f'AboutStatistic already exists: {statistic.number} - {statistic.label}'))
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created WhoWeAre: {who_we_are.title}'))
        else:
            self.stdout.write(self.style.WARNING(f'WhoWeAre already exists: {who_we_are.title}'))
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created AboutStatisticSection: {statistic_section.title}'))
        else:
            self.stdout.write(self.style.WARNING(f'AboutStatisticSection already exists: {statistic_section.title}'))

        # Create About Statistic Items
        statistic_items_data = [
            {'icon': 'People', 'value': '500+', 'label': 'Happy Clients', 'color': '#EE1C27'},
            {'icon': 'TrendingUp', 'value': '10+', 'label': 'Years Experience', 'color': '#EE1C27'},
            {'icon': 'Verified', 'value': '100%', 'label': 'Success Rate', 'color': '#EE1C27'},
            {'icon': 'Gavel', 'value': '1000+', 'label': 'Cases Handled', 'color': '#EE1C27'},
        ]
        ...
        for item_data in statistic_items_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created AboutStatisticItem: {item.value} - {item.label}'))
            else:
                self.stdout.write(self.style.WARNING(f'AboutStatisticItem already exists: {item.value} - {item.label}'))
        ...
        for value_data in values_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created AboutValue: {value.title}'))
            else:
                self.stdout.write(self.style.WARNING(f'AboutValue already exists: {value.title}'))
        ...
        for achievement_data in achievements_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created AboutAchievement: {achievement.title}'))
            else:
                self.stdout.write(self.style.WARNING(f'AboutAchievement already exists: {achievement.title}'))
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created AboutCTA: {about_cta.title}'))
        else:
            self.stdout.write(self.style.WARNING(f'AboutCTA already exists: {about_cta.title}'))
