...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created ContactHero: {contact_hero.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"ContactHero already exists: {contact_hero.title}")
            )
        ...
        for stat_data in hero_statistics_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created ContactStatistic: {statistic.value} - {statistic.label}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"ContactStatistic already exists: {statistic.value} - {statistic.label}"
                    )
                )
        ...
        contact_info_data = [
            {
                "icon": "Phone",
                "title": "Phone",
                "details": "+92 328 5007444",
                "subtitle": "Mon-Fri 9am to 6pm",
            },
            {
                "icon": "Email",
                "title": "Email",
                "details": "info@paktaxlawyer.com",
        ...
        for info_data in contact_info_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created ContactInfo: {contact_info.title}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"ContactInfo already exists: {contact_info.title}"
                    )
                )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created ContactForm: {contact_form.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"ContactForm already exists: {contact_form.title}")
            )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created ContactMap: {contact_map.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"ContactMap already exists: {contact_map.title}")
            )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created ContactCTA: {contact_cta.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"ContactCTA already exists: {contact_cta.title}")
            )
