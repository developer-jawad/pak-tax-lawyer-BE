...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            self.stdout.write(
                self.style.WARNING(f"FooterBrand already exists: {brand.name}")
            )
        ...
        social_links_data = [
            {
                "platform": "Facebook",
                "url": "https://www.facebook.com/profile.php?id=61555804476159",
            },
            {"platform": "Instagram", "url": "https://www.instagram.com"},
            {
                "platform": "YouTube",
                "url": "https://www.youtube.com/@PakTaxLawyer/videos",
            },
        ...
        for social_data in social_links_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created FooterSocialLink: {social_link.platform}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"FooterSocialLink already exists: {social_link.platform}"
                    )
                )
        ...
        for quick_data in quick_links_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created FooterQuickLink: {quick_link.label}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"FooterQuickLink already exists: {quick_link.label}"
                    )
                )
        ...
        for service_data in service_links_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created FooterServiceLink: {service_link.label}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"FooterServiceLink already exists: {service_link.label}"
                    )
                )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created FooterContact: {contact.title}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"FooterContact already exists: {contact.title}")
            )
        ...
        for legal_data in legal_links_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created FooterLegalLink: {legal_link.label}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"FooterLegalLink already exists: {legal_link.label}"
                    )
                )
